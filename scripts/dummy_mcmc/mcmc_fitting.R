...
# true_vals_epid_1 <- c(unknown_pars$epid_parameters_s1$transmissibility, unique(unknown_pars$epid_parameters_s1$susceptibility),
#                       unknown_pars$epid_parameters_s1$start_date -
#                         as.Date(paste0('01-01-',year(unknown_pars$epid_parameters_s1$start_date)), '%d-%m-%Y'))
# epidemic_to_fit <- surveillance_data %>% filter(index==1); llikelihood(true_vals_epid_1)
# system.time(llikelihood(true_vals_epid_1))
...
mcmc_parallel <- function(i){
  ...
  run_mcmc_inference(
    ...
    care_delays = delays,
    initial_parameters = c(0.1, 0.5, 0.5, 0.5, 240), # c(transmissibility, 3x susceptibility, start day)
